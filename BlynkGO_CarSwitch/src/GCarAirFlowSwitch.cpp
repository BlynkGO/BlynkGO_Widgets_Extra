...
#define LV_OBJX_NAME  "GCarAirFlowSwitch"
...
static lv_res_t airflow_indic_signal_cb(lv_obj_t *obj, lv_signal_t sign, void* param);
...
static void airflow_switch_refresh(GCarAirFlowSwitch *pCarAirFlowSW);
...
static lv_res_t GSignal_cb(lv_obj_t *obj, lv_signal_t sign, void* param)
{
  ...
  if(sign == LV_SIGNAL_CLEANUP) {
    Serial.println("[GCarAirFlowSwitch] signal cleanup");
    GCarAirFlowSwitch* pCarAirFlowSW = (GCarAirFlowSwitch*) lv_obj_get_user_data(obj);

    gcar_airflow_sw_ext_t *ext = (gcar_airflow_sw_ext_t *) pCarAirFlowSW->ext_attr();
    free_widget(ext->knob);
    free_widget(ext->indic);
    
    if(pCarAirFlowSW->hasLocalStyle()) {pCarAirFlowSW->freeLocalStyle(); } //pLed->_has_localstyle = false; } // 
    pCarAirFlowSW->_created = false;
  }
  return LV_RES_OK;
}


GCarAirFlowSwitch::GCarAirFlowSwitch(GWidget& parent) : GObject(&parent) {}
GCarAirFlowSwitch::GCarAirFlowSwitch(GWidget* parent) : GObject(parent) {}
GCarAirFlowSwitch::~GCarAirFlowSwitch() { this->del(); }

void GCarAirFlowSwitch::create(){
  ...
  if(!this->isCreated()) {
    ...
    if(this->obj != NULL) {
      ...
      lv_img_set_src(this->obj, (void*) &pic_car_airflow_switch);
      this->clickable(false);
   
      gcar_airflow_sw_ext_t *ext = (gcar_airflow_sw_ext_t*) this->alloc_ext_attr(sizeof(gcar_airflow_sw_ext_t));
      ext->angle_offset  = -116;
      ext->max_level     = 4;
      ext->level         = 0;
      ...
      ext->indic->signal_cb(airflow_indic_signal_cb);
...
void GCarAirFlowSwitch::create(GWidget& parent){
  this->_par = &parent;
  create();
}
...
void GCarAirFlowSwitch::selected(uint8_t selected_id){
  if(!this->isCreated()) this->create();
  gcar_airflow_sw_ext_t *ext = (gcar_airflow_sw_ext_t *) this->ext_attr();
  selected_id = constrain(selected_id, 0, ext->max_level);
  
  int knob_angle = (selected_id)* abs(ext->angle_offset)/2 + ext->angle_offset;
  ext->indic_angle = ANGLE360(90 - knob_angle);
  airflow_switch_refresh(this);
}

uint8_t GCarAirFlowSwitch::selected_id(){
  if(!this->isCreated()) this->create();
  gcar_airflow_sw_ext_t *ext = (gcar_airflow_sw_ext_t *) this->ext_attr();
  return ext->level;
}
...
static lv_res_t airflow_indic_signal_cb(lv_obj_t* indic_obj, lv_signal_t sign, void* param)
{
  lv_res_t res;
  ...
  GCarAirFlowSwitch* pCarAirFlowSW = (GCarAirFlowSwitch*) indic->_par;
  gcar_airflow_sw_ext_t *ext = (gcar_airflow_sw_ext_t *) pCarAirFlowSW->ext_attr();
  ...
  if( sign == LV_SIGNAL_CORD_CHG) { 
    ...
    lv_point_t pivot                  = { pCarAirFlowSW->width()/2, pCarAirFlowSW->height()/2 };
    ...
    airflow_switch_refresh(pCarAirFlowSW);    
  }else
...
static void airflow_switch_refresh(GCarAirFlowSwitch *pCarAirFlowSW){
  ...
  if(ext->level != level ){
    ext->level = level;
    pCarAirFlowSW->event_send(EVENT_VALUE_CHANGED, NULL);    // อัพเดต level สู่ระบบ พร้อมส่ง EVENT_VALUE_CHANGED ออกไป
  }
}


GCarAirFlowSwitch* new_GCarAirFlowSwitch(GWidget& parent){
  GCarAirFlowSwitch* w = (GCarAirFlowSwitch*) esp32_malloc(sizeof(GCarAirFlowSwitch));
  new (w) GCarAirFlowSwitch(parent);
  return w;
}
