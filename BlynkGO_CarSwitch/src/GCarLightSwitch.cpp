...
#define LV_OBJX_NAME  "GCarLightSwitch"
...
static lv_res_t light_indic_signal_cb(lv_obj_t *obj, lv_signal_t sign, void* param);
...
static void light_switch_refresh(GCarLightSwitch *pCarLightSW);
...
static lv_res_t GSignal_cb(lv_obj_t *obj, lv_signal_t sign, void* param)
{
  ...
  if(sign == LV_SIGNAL_CLEANUP) {
    Serial.println("[GCarLightSwitch] signal cleanup");
...
GCarLightSwitch::GCarLightSwitch(GWidget& parent) : GObject(&parent) {}
GCarLightSwitch::GCarLightSwitch(GWidget* parent) : GObject(parent) {}
GCarLightSwitch::~GCarLightSwitch() { this->del(); }

void GCarLightSwitch::create(){
  ...
  if(!this->isCreated()) {
    ...
    if(this->obj != NULL) {
      ...
      lv_img_set_src(this->obj, (void*) &pic_car_light_switch);
      this->clickable(false);
   
      gcar_light_sw_ext_t *ext = (gcar_light_sw_ext_t*) this->alloc_ext_attr(sizeof(gcar_light_sw_ext_t));
      ext->angle_offset  = -60;
      ext->max_level     = 5;
      ext->level         = 0;
      ...
      ext->knob->setImage(pic_car_light_knob);
      ...
      ext->indic_arm_length = (ext->knob->width() - ext->indic->width())/2 - 20;

      if(ancestor_indic_signal_cb == NULL) ancestor_indic_signal_cb =  ext->indic->signal_cb();
      ext->indic->signal_cb(light_indic_signal_cb);
...
void GCarLightSwitch::create(GWidget& parent){
  this->_par = &parent;
  create();
}
...
void GCarLightSwitch::selected(uint8_t selected_id){
  if(!this->isCreated()) this->create();
  gcar_light_sw_ext_t *ext = (gcar_light_sw_ext_t *) this->ext_attr();
  ...
  light_switch_refresh(this);
}

uint8_t GCarLightSwitch::selected_id(){
  if(!this->isCreated()) this->create();
  gcar_light_sw_ext_t *ext = (gcar_light_sw_ext_t *) this->ext_attr();
  return ext->level;
}
...
static lv_res_t light_indic_signal_cb(lv_obj_t* indic_obj, lv_signal_t sign, void* param)
{
  lv_res_t res;
  ...
  GCarLightSwitch* pCarLightSW = (GCarLightSwitch*) indic->_par;
  gcar_light_sw_ext_t *ext = (gcar_light_sw_ext_t *) pCarLightSW->ext_attr();
  ...
  if( sign == LV_SIGNAL_CORD_CHG) { 
    ...
    lv_point_t pivot                  = { pCarLightSW->width()/2, pCarLightSW->height()/2 };
...
//    // กำหนดมุม ระหว่าง angle_start องศา ถึง angle_end องศา (กวาดมุมแบบนับตามเข็ม) แต่มุมนับทวนเข็มตามระบบ xy
    ...
    float angle_end   = 360 + (90+ ext->angle_offset/2 * 3);
    ...
    light_switch_refresh(pCarLightSW);    
  }else
  if( sign == LV_SIGNAL_RELEASED  || sign == LV_SIGNAL_PRESS_LOST) {
    ...
    uint16_t angle_max_from_offset = ANGLE360(5*abs(ext->angle_offset)/2);
    uint8_t level = round( map_f ( angle_from_offset, 0, angle_max_from_offset, 0, ext->max_level));

    pCarLightSW->selected(level);
  }
...
static void light_switch_refresh(GCarLightSwitch *pCarLightSW){
  ...
  uint16_t angle_max_from_offset = ANGLE360(5*abs(ext->angle_offset)/2);       // มุมหมุนภาพ ที่มากสุด นับจาก angle_offset
  uint8_t level = round( map_f ( angle_from_offset, 0, angle_max_from_offset, 0, ext->max_level));  // แปลง level ออกมา
  if(ext->level != level ){
    ext->level = level;
    pCarLightSW->event_send(EVENT_VALUE_CHANGED, NULL);    // อัพเดต level สู่ระบบ พร้อมส่ง EVENT_VALUE_CHANGED ออกไป
  }
}


GCarLightSwitch* new_GCarLightSwitch(GWidget& parent){
  GCarLightSwitch* w = (GCarLightSwitch*) esp32_malloc(sizeof(GCarLightSwitch));
  new (w) GCarLightSwitch(parent);
  return w;
}
